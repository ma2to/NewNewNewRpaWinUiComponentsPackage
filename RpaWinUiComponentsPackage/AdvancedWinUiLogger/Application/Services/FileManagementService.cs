...
/// <summary>
/// APPLICATION SERVICE: File management operations with rotation and cleanup
/// CLEAN ARCHITECTURE: Application layer service for file management business logic
/// ENTERPRISE: Professional file management with comprehensive policies
/// </summary>
internal sealed class FileManagementService : IFileManagementService
{
    ...
    public FileManagementService(ILoggerRepository repository)
    {
    ...
    /// <summary>
    /// ENTERPRISE: Check if log file should be rotated
    /// MONITORING: Size-based rotation decision logic
    /// </summary>
    public async Task<Result<bool>> ShouldRotateLogFileAsync(string filePath, long maxSizeBytes, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var shouldRotate = fileSizeResult.Value >= maxSizeBytes;
    ...
    /// <summary>
    /// ENTERPRISE: Perform advanced rotation with configuration
    /// ADVANCED: Full rotation workflow with configuration support
    /// </summary>
    public async Task<Result<RotationResult>> PerformAdvancedRotationAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // Use the FileRotationService for advanced rotation
            ...
            if (configuration.EnableAutoCleanup)
            {
                ...
                if (cleanupResult.IsFailure)
                {
                    // Rotation succeeded but cleanup failed - log warning but don't fail
                    var rotation = rotationResult.Value;
                    var updatedRotation = rotation with
                    {
                        ErrorMessage = $"Rotation succeeded but cleanup failed: {cleanupResult.Error}"
                    };
                    return Result<RotationResult>.Success(updatedRotation);
                }
            }
        ...
        {
            return Result<RotationResult>.Failure($"Advanced rotation failed: {ex.Message}", ex);
        }
    }
}