...
internal sealed class ExportService : IExportService
{
    ...
    private readonly SmartAddDelete.Interfaces.ISmartOperationService? _smartOperationService;
    ...
    public ExportService(
        ...
        SmartAddDelete.Interfaces.ISmartOperationService? smartOperationService = null)
    {
        ...
        // SmartOperationService je optional (pre cleanup pred export)
        _smartOperationService = smartOperationService;
    }
    ...
    public async Task<InternalExportResult> ExportAsync(InternalExportDataCommand command, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            // CLEANUP PRED EXPORTOM (remove empty rows based on grace period)
            if (_smartOperationService != null)
            {
                ...
                var cleanupConfig = new Core.ValueObjects.RowManagementConfiguration
                {
                    ...
                    UserEmptyRowGracePeriod = _options.UserEmptyRowGracePeriod,
                    CleanupAutoGeneratedEmptyRows = _options.CleanupAutoGeneratedEmptyRows
                };
    ...
    /// <summary>
    /// Konvertuje dáta do DataTable formátu s ArrayPool optimalizáciou
    /// Vytvorí DataTable so stĺpcami z prvého riadku a pridá všetky dáta po dávkach
    /// Uses ArrayPool to minimize LOH allocations for large datasets
    /// </summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (data.Count > 0)
        {
            ...
            // ArrayPool: Rent buffer (minimalizuje LOH alokácie)
            var buffer = System.Buffers.ArrayPool<object?>.Shared.Rent(columnCount);
            ...
            try
            {
                ...
                for (int i = 0; i < data.Count; i += batchSize)
                {
                    ...
                    for (int j = i; j < batchEnd; j++)
                    {
                        ...
                        {
                            var columnName = columnNames[colIdx];
                            if (data[j].TryGetValue(columnName, out var value))
                            {
                                buffer[colIdx] = value ?? DBNull.Value;
                            }
                            else
                            {
                                buffer[colIdx] = DBNull.Value;
                            }
                        }

                        // Add row to DataTable using buffer
                        var dataRow = dataTable.NewRow();
                        dataRow.ItemArray = buffer.Take(columnCount).ToArray(); // Copy only needed elements
                        dataTable.Rows.Add(dataRow);

                        // Clear buffer for security (prevent data leakage)
            ...
            finally
            {
                ...
                System.Buffers.ArrayPool<object?>.Shared.Return(buffer, clearArray: true);
            }
        }

        _logger.LogDebug("Converted {RowCount} rows to DataTable for operation {OperationId} (with ArrayPool optimization)",
