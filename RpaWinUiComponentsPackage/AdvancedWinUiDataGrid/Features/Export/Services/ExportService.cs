...
internal sealed class ExportService : IExportService
{
    ...
    // REMOVED: SmartOperationService - no longer needed (optional parameter, was not used)
    ...
    public async Task<InternalExportResult> ExportAsync(InternalExportDataCommand command, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            // NEW ARCHITECTURE: No cleanup needed before export
            // Data shifting handles row management automatically
            // Export works directly with actual data in storage layer
            // REMOVED: SmartAddDelete cleanup logic (EnsureMinRowsAndLastEmptyAsync)
